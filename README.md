...
- `data/actuals.json` — contest size + Circa's posted selections per leg (entries per team, won/lost/pending). Written by the `update-actuals` workflow from Circa's Selections PDF and ESPN scores; the app's editor is a manual override.
...
- `scripts/fetch-odds.mjs`, `scripts/fit-ratings.mjs`, `scripts/fetch-actuals.mjs` — the data jobs (The Odds API, nflverse, Circa's PDFs, ESPN).
...
- `.github/workflows/update-actuals.yml` — every 3 h Thu–Mon: read Circa's weekly Selections PDF (every entry's pick) and ESPN's scoreboard (results), commit, redeploy.
...
3. Nothing after lock: Circa's picks and the results arrive on their own (the Actuals editor remains as a manual override).
...
npm test
```
To run the data jobs locally: `ODDS_API_KEY=… node scripts/fetch-odds.mjs && node scripts/fit-ratings.mjs`, and `node scripts/fetch-actuals.mjs` (needs `pdftotext` from poppler).
