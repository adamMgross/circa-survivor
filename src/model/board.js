import { ALL_TEAMS, OPP } from "../schedule.js";
import { lineFor, marketLine } from "./lines.js";
import { computeEV, computeExactEV, computeDili, fvFor, gauntletFeasible } from "./value.js";
...
// Stats for the selected leg for one entry's picks, with deltas against the previous refresh and the best five in
// each ranked column. A pick that breaks holiday feasibility is marked infeasible and gets no DILI or flag.
export function boardStats(legId, data, params, picks, style) {
  const usedBy = {};
  for (const [leg, team] of Object.entries(picks)) usedBy[team] = leg;
  const burned = new Set(Object.keys(usedBy).filter((t) => usedBy[t] !== legId));
  const refused = new Set(Object.keys(OPP[legId]).filter((t) => !burned.has(t) && !gauntletFeasible(picks, legId, t)));
  ...
  const TOP = 5;
  const top = (key) => new Set(ALL_TEAMS.filter((t) => !refused.has(t) && diliRows[t][key] != null).sort((a, b) => diliRows[b][key] - diliRows[a][key]).slice(0, TOP));
  const flags = [["dili", "diliTop"], ["ev", "evTop"], ["evx", "evxTop"], ["win", "winTop"]].map(([key, flag]) => [flag, top(key)]);
  const rows = {};
  for (const t of ALL_TEAMS) {
    const a = diliRows[t], b = prevRows?.[t];
    rows[t] = { ...a, ...(refused.has(t) ? { infeasible: true, dili: null } : {}), ...(b ? {
      ...
      dDili: !refused.has(t) && a.dili != null && b.dili != null ? a.dili - b.dili : null,
    } : {}) };
