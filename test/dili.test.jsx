...
const mk = () => { const rows = {}; for (const t of ALL_TEAMS) { const ln = data.legs.W2.lines[t]; rows[t] = { win: ln?.win ?? null, pick: ln && ln.win > 0.5 ? 1 / 16 : 0, fv: fvFor("W2", t, data) }; } return computeEV("W2", rows).rows; };

let rows = computeDili("W2", mk(), data, new Set(), "balanced").rows;
...
ok("burned teams get no DILI", (() => { const r = computeDili("W2", mk(), data, new Set(["KC"]), "balanced").rows; return r.KC.dili == null && r.BUF.dili != null; })());

// style: Future punishes studs harder than Now; calendar: the same stud is punished less late in the season
const rNow = computeDili("W2", mk(), data, new Set(), "now").rows, rFut = computeDili("W2", mk(), data, new Set(), "future").rows;
...
ok("late-season k is small", (() => { const r = {}; for (const t of ALL_TEAMS) r[t] = { win: late.legs.W17.lines[t]?.win ?? null, pick: 1 / 16, fv: 0 }; const { k } = computeDili("W17", computeEV("W17", r).rows, late, new Set(), "future"); return k < 0.5; })());

ok("computeEV and computeDili leave their input rows untouched", (() => {
  const input = mk(), before = structuredClone(input);
  const out = computeDili("W2", computeEV("W2", input).rows, data, new Set(["KC"]), "balanced").rows;
  return isDeepStrictEqual(input, before) && out.BUF !== input.BUF && out.BUF.dili != null && input.BUF.dili === undefined;
})());
...
const holiday = (burnedList, style = "balanced") => computeDili("W2", mk(), data, new Set(burnedList), style).rows;
let h = holiday([]);
...
ok("no self-dock when picking on the holiday week", (() => { const r = {}; for (const t of Object.keys(OPP.XM)) r[t] = { win: 0.7, pick: 1 / 4, fv: 0 }; return computeDili("XM", computeEV("XM", r).rows, data, new Set(), "balanced").rows.PHI.holidayParts.length === 0; })());
